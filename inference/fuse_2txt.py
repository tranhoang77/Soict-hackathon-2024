...
def read_results_from_txt(filename: Path, thresh=0.1):
...
def save_wbf_predictions(image_name, boxes, scores, labels, output_folder):
...
def parse_arguments():
    ...
    parser.add_argument('--path_data_test', type=str, required=True,
                        help='Path to the public test directory containing images.')
    parser.add_argument('--output_folder', type=str, required=True,
                        help='Path to the output file for fused predictions.')
    return parser.parse_args()

def main():
    # Parse arguments
    args = parse_arguments()
    data_test = Path(args.path_data_test)
    output_folder = args.output_folder
    if not os.path.exists(output_folder):
        os.mkdir(output_folder)
    # Các file đầu vào
    filenames = [
        Path("predict_step3_fuse_base_to_remove_person/nafgs_10e_01_remove_person_015.txt"),
        Path("predict_step3_fuse_base_to_remove_person/pseudo_15e_01_remove_person_015.txt"),

    ]
    
    weights = [1, 1]
    # weights = [3, 3, 2, 2, 2, 1, 2, 1, 1, 1]
    output_folder += ("/nafgs_10e_withDat_01_remove_person_015-pseudo_15e_01_remove_015_1-1_iou-0.55_skip_box-0.2.txt")
    if os.path.exists(output_folder):
        os.remove(output_folder)
    print(output_folder)
    # iou_thr = 0.7
    iou_thr = 0.55  # Ngưỡng IoU cho BF
    skip_box_thr = 0.2  # Confidence threshold để bỏ qua box
    sigma = 0.1

    print("Read data from TXT files")
    # Đọc dữ liệu từ tất cả các file đầu vào
    data_list = [read_results_from_txt(filename) for filename in filenames]

    print("Processing each image in data")
    for image_path in sorted(data_test.glob("*.jpg")):
        ...
        if any(boxes_list):
            ...
            # Lưu kết quả dự đoán
            save_wbf_predictions(image_path.name, boxes, scores, labels, output_folder)
