...
CHECKPOINTS=(
    "weight/naf_gs_pseudo_e55/epoch55.pt"
    "weight/yolo_naf_gs_e60/epoch60.pt"
)

# Danh sách các file output
OUTPUTS=(
    "../../../inference/results/naf_gs_pseudo_e55.txt"
    "../../../inference/results/yolo_naf_gs_e60.txt"
)

# Kiểm tra tính nhất quán của mảng
if [ ${#CHECKPOINTS[@]} -ne ${#OUTPUTS[@]} ]; then
    echo "Lỗi: Số lượng checkpoints và outputs không khớp nhau."
    exit 1
fi
...
run_task() {
    ...
    CUDA_VISIBLE_DEVICES="$gpu_id" python3 predict.py --checkpoint_file "$checkpoint_file" --input_dir ../../data/public_test_nafnet_gsad --output_file "$output_file"
}
